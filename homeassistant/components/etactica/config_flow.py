...
class eTacticaFlowHandler(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_zeroconf(
        ...
        # EG hostname's aren't a great fallback, but they are uniquely preconfigured
        # if a user explicitly changes hostname, they should understand things may break
        self.identifier = discovery_info["properties"].get("gid", self.hostname)
        await self.async_set_unique_id(self.identifier)
        _LOGGER.info("karl: set unique id to %s, name: %s", self.identifier, self.name)
        # self._abort_if_unique_id_configured()
